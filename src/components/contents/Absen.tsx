...
export default function AbsenSiswa() {
  ...
  const [mapelOptions, setMapelOptions] = useState<{ id: number; namaMapel: string; memberKelasId?: number }[]>([]);
  ...
  const handleSiswaChange = async (selected: any) => {
    ...
    setMapelOptions([]);
    try {
      // Definisikan tipe yang mencakup memberKelasId
      const mapelList: { id: number; namaMapel: string; memberKelasId: number }[] = [];
  
      // Gunakan pendekatan yang sama dengan NilaiSiswa.tsx
      if (selected?.value?.MemberKelas) {
        selected.value.MemberKelas.forEach((memberKelas: any) => {
          if (
            memberKelas.Kelas &&
            ...
            memberKelas.Kelas.MapelKelas.forEach((mapel: any) => {
              mapelList.push({
                ...
                memberKelasId: memberKelas.id // Simpan memberKelasId yang sesuai
              });
            });
          }
        });
      ...
      // Log untuk debugging
      console.log("MemberKelas dari siswa:", selected?.value?.MemberKelas);
      console.log("Mapel list yang terbentuk:", mapelList);
...
// Modifikasi handleAbsensiChange untuk menyimpan memberKelasId
const handleAbsensiChange = (mapelId: number, field: "tanggal" | "status" | "memberKelasId", value: string | number) => {
  setAbsensi((prev) => ({
    ...prev,
    [mapelId]: {
      ...prev[mapelId],
...
const handleSubmit = async (mapelId: number) => {
  ...
  // Dapatkan memberKelasId yang sesuai dengan mapel ini
  const mapel = mapelOptions.find(m => m.id === mapelId);
  
  if (!mapel || !mapel.memberKelasId) {
    toast.error("Tidak dapat menemukan data kelas untuk mata pelajaran ini.");
    return;
  }
  ...
  try {
    ...
    const dataToSend = {
      ...
      memberKelasId: mapel.memberKelasId
    };
