...
export default function PencapaianSiswa() {
  ...
  const fetchStudents = async () => {
    try {
      const result = await getAllStudents(1);
  ...
  const handleSearchChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (query.length > 0) {
      ...
      fetchStudents(); // jika input dikosongkan, muat ulang semua siswa
    }
  };
  ...
  const handleSelectStudent = (student: Siswa) => {
    setForm({
      ...form,
      studentProfilId: student.id, // Menetapkan siswa yang dipilih ke form
    });
  };
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      ...
      <div className="max-w-4xl mx-auto bg-white shadow-lg rounded-lg p-6">
        ...
        {/* Pilih Siswa dari Pencarian */}
        {form.studentProfilId === 0 ? (
          <div className="mb-4">
            <p className="text-red-500">
              ❌ Mohon pilih siswa terlebih dahulu.
            </p>
          </div>
        ) : (
          <div className="mb-4">
            <p className="font-semibold">
              Siswa yang dipilih:{" "}
              {students.find((s) => s.id === form.studentProfilId)?.nama}
            </p>
          </div>
        )}
